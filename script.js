...
Chart.rose = function () {
	...
	function formatData(data) {
        ...
        // Needed for non-deterministic accessors
		...
		// Now convert the area values to radiius
...
Chart.legend = function (entries) {
	...
	legend.entries = legend.canvas.selectAll('.legend-entry')
		...
		.attr('transform', function (d, i) {
			if (i < 4)
				return 'translate(' + (symbolRadius + i * 80) + ', ' + (height / 2) + ')';
			else
				return 'translate(' + (symbolRadius + (i - 4) * 80) + ', ' + (height) + ')';
		});
...
Chart.slider = function (minVal, maxVal, step) {
	...
	d3.select("input").on("change", function () {
		...
		d3.selectAll('.wedgeGroup')
			.filter(function (d, i) { if (i >= 17) i -= 17; return i < value; })
			.transition()
			.duration(500)
			.attr('transform', 'scale(1,1)');
